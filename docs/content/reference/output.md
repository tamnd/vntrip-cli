...
describes what users get. Pick a format with `-o`, or let vntrip choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vntrip <command> -o table   # aligned columns for reading
vntrip <command> -o jsonl   # one JSON object per line, for piping
vntrip <command> -o json    # a single JSON array
vntrip <command> -o csv     # spreadsheet friendly
vntrip <command> -o tsv     # tab-separated
vntrip <command> -o url     # just the URL column
vntrip <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vntrip <command> --fields id,title,url
```
...
```bash
vntrip <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vntrip <command>            # a table, because this is a terminal
vntrip <command> | wc -l    # JSONL, because this is a pipe
```
